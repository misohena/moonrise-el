...
The function is called with a calendar date (month day year) as the
argument and should return a string for display.
...
Get the date from the dynamic variable `org-agenda-current-date' or
`date'.
...
when available to improve performance."
  ...
  (with-no-warnings (defvar date))
  (moonrise-org-agenda-make-string-with-cache
   (or org-agenda-current-date date)))
;;(moonrise-org-agenda)

(defun moonrise-org-agenda-make-string-with-cache (date)
  "Create a moon events string for DATE, using cache if enabled.

DATE is a calendar date (month day year).

If `moonrise-org-agenda-use-cache' is non-nil, return cached data
when available, otherwise compute and cache the result."
  (if moonrise-org-agenda-use-cache
      (let ((key (moonrise-org-agenda-cache-key date)))
        (or (moonrise-org-agenda-cache-get key)
            (moonrise-org-agenda-cache-put
             key
             (moonrise-org-agenda-make-string date))))
    (moonrise-org-agenda-make-string date)))

(defun moonrise-org-agenda-make-string (date)
  "Create a moon events string for DATE in Org Agenda.

DATE is a calendar date (month day year).

This function calls `moonrise-org-agenda-format-function' with DATE."
  (funcall moonrise-org-agenda-format-function date))
...
(defun moonrise-org-agenda-cache-key (date)
  "Generate a cache key for DATE in Org Agenda.
...
Return nil if not found."
  (cdr (assoc key moonrise-org-agenda-cache)))
